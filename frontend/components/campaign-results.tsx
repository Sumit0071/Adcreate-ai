...
export default function CampaignResults( { brief, ads, onBack }: CampaignResultsProps ) {
  ...
  const generateReport = () => {
    ...
    text += `Target: ${brief.targetAudience}\n`;
    text += `Location: ${brief.location}\n`;
    text += `Offer: ${brief.offer}\n`;
    text += `${"=".repeat( 70 )}\n\n`;
    ...
    editedAds.forEach( ( ad, i ) => {
      ...
      text += `Quality Score : ${score.overall}/${score.maxOverall} (${score.grade})\n`;
      text += `Policy Status : ${policy.status}\n`;
  ...
  const exportTXT = () => {
    ...
    a.href = url;
    a.download = `${brief.businessName.replace( /\s+/g, "_" )}.txt`;
    a.click();
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize( 18 );
    doc.text( "Campaign Report", 15, 15 );

    doc.setFontSize( 10 );
    doc.text( generateReport(), 15, 30, {
      maxWidth: 180,
    } );

    doc.save(
      `${brief.businessName.replace( /\s+/g, "_" )}.pdf`
    );
          ...
          <Card key={idx} className={`overflow-hidden ${isDark ? "bg-gray-800/50 border-gray-700" : ""}`}>
            ...
            <div className={`px-5 py-4 border-b flex items-center justify-between ${isDark ? "border-gray-700" : "border-gray-100"}`}>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold ${idx % 3 === 0 ? "bg-indigo-600" : idx % 3 === 1 ? "bg-purple-600" : "bg-pink-600"
                  }`}>
                  {idx + 1}
                </div>
                <div>
                  ...
                  {policy.violations.length > 0 ? (
                    <div className="space-y-2">
                      {policy.violations.map( ( v, i ) => (
                        <div key={i} className={`p-3 rounded-lg border text-sm ${v.severity === "high" ? "bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800" :
